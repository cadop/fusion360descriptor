...
class Joint:
    ...
    def joint_xml(self):
        ...
        origin.attrib = {'xyz':' '.join([str(_ * Joint.joint_scale) for _ in self.xyz]), 'rpy':'0 0 0'}
...
class Link:
    ...
    def link_xml(self):
        ...
        # model_ext = Link.model_ext # 'obj' # 'stl' or 'obj'
        ...
        origin_i.attrib = {'xyz':' '.join([str(_*Link.mesh_scale) for _ in self.center_of_mass]), 'rpy':'0 0 0'}       
        ...
        if self.sub_mesh: # if we want to export each as a separate mesh
            for body_name in self.body_dict[self.name]:
                ...
                mesh_v.attrib = {'filename':f'package://{self.sub_folder}{body_name}.{Link.model_ext}','scale':f'{Link.stl_scale} {Link.stl_scale} {Link.stl_scale}'}
            ...
            geometry_v = SubElement(visual, 'geometry')
            mesh_v = SubElement(geometry_v, 'mesh')
            mesh_v.attrib = {'filename':f'package://{self.sub_folder}{self.name}.{Link.model_ext}','scale':f'{Link.stl_scale} {Link.stl_scale} {Link.stl_scale}'}
            material = SubElement(visual, 'material')
            material.attrib = {'name':'silver'}
        ...
        mesh_c.attrib = {'filename':'package://' + self.sub_folder + self.name + f'.{Link.model_ext}','scale':f'{Link.stl_scale} {Link.stl_scale} {Link.stl_scale}'}
